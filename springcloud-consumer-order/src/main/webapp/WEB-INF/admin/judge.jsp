...
<script type="text/html" id="toolbarDemo">
    <div>
        ...
        <form style="margin:10px;display:inline;" action="<%=request.getContextPath()%>/Uploadjudge" method="post"
...
<script>
    layui.use(['table'], function () {//使用layui的数据表格组件table
        ...
        table.render({
            ...
            , title: '题库'
            , url: '<%=request.getContextPath()%>/alljudge', //数据接口，为json格式，使用jsp获取获取项目名称
        ...
        function bindTableToolbarFunction() {
            $('#btnAdd').click(function () {//定义表格头部单击添加事件
                ...
                var index = layer.open({
                    ...
                    area: ['50%', '90%'],
                    offset: ['5%', '25%'],
                    content: '<%=request.getContextPath()%>/addjudge',
                    end: function () {
            ...
            $('#btnDel').click(function () {//点击删除按钮事件
                ...
                if (data.length == 0) {
                    ...
                    layer.confirm('真的删除序号为' + JSON.stringify(arrId) + '的记录吗？', function (index) {
                        ...
                        $.ajax({//无刷新提交
                            url: '<%=request.getContextPath()%>/deletejudge',
        ...
        table.on('tool(test)', function (obj) {//设置表格标签的属性lay-filter="test"
            ...
            if (obj.event === 'del') {
                ...
                var index = layer.open({
                    ...
                    area: ['50%', '90%'],
                    offset: ['5%', '25%'],
                    content: '<%=request.getContextPath()%>/updatejudgejsp?id=' + data.id,
                    end: function () {
